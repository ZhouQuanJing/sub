...
```bash
bash <(curl -Ls [https://raw.githubusercontent.com/mhtsll/3x-ui/main/install.sh](https://raw.githubusercontent.com/mhtsll/3x-ui/main/install.sh))
```
...
```bash
wget -N [https://gitlab.com/fscamen/warp/-/raw/main/menu.sh](https://gitlab.com/fscamen/warp/-/raw/main/menu.sh) && bash menu.sh
```
...
1. 在 VS Code 中按 `Ctrl + ,`（Mac 下为 `Cmd + ,`）打开设置。
2. 搜索 `Http: Proxy`，明确填入你本地的代理，例如：`http://127.0.0.1:10809`。
3. 搜索 `Http: Proxy Strict SSL`，**务必取消勾选（将其设为 False）**。
4. **完全重启 VS Code**，回到 Cline 面板点击 Retry，测试 Gemini / Claude 模型连通性。
